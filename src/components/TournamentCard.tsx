...
export default function TournamentCard({ t }: { t: Tournament }) {
  ...
  return (
    <Link href={`/tournaments/${t.slug}`} className="glass card-hover group block overflow-hidden">
      <CoverArt emoji={t.emoji} src={`/images/tournaments/${t.slug}.webp`} gradient="violet" className="h-40 transition-transform duration-500 group-hover:scale-105" alt={t.title} />
      <div className="p-4">
