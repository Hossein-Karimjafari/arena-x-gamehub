      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {posts.map((p, i) => (
          <Link key={p.id} href={`/blog/${p.slug}`} className="glass card-hover group overflow-hidden">
            <CoverArt emoji={p.emoji} src={`/images/blog/${p.slug}.webp`} gradient={gradients[i % gradients.length]} className="h-40 transition-transform duration-500 group-hover:scale-105" alt={p.title} />
            <div className="p-5">
